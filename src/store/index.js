...
/* (middleware - усилитель именно для dispatch - набор обложек из функций, которые по цепочке принимают store(можно декомпозировать - использует только dispatch и getState), dispatch(заменен на next - миддлверы зачастую подключаются цепочкой, и next запускает следующий миддлвер(с помощью applyMiddleware), суммируя усиления) и action, внутри пишем функционал по усилению и возвращаем диспетчер, подключаем все это при создании store(ниже)) */
